...
class Fileglancer:
    ...
    def _resolve(self, path: str) -> Tuple[str, str]:
        ...
        if best_fsp is None:
            if not self.file_share_paths():
                # Distinguished from a genuine mismatch, which otherwise sends
                # people looking at their own path when the server simply has
                # nothing mounted.
                raise FileglancerError(
            ...
            # Deliberately does not enumerate the shares: a real deployment has
            # hundreds, which buries the message it is attached to.
            raise FileglancerError(
                f"No file share matches {path!r}. "
                f"Call file_share_paths() to see the available shares.")

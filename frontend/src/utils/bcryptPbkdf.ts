/**
 * bcrypt_pbkdf - Password-Based Key Derivation Function using bcrypt
 *
 * This is the key derivation function used by OpenSSH for encrypting private keys.
 * It uses the Blowfish cipher internally with a modified expensive key schedule.
 *
 * Reference: https://github.com/openssh/openssh-portable/blob/master/openbsd-compat/bcrypt_pbkdf.c
 * Ported from bcrypt-pbkdf (npm) which is ported from OpenBSD.
 */

// =============================================================================
// Blowfish S-boxes and P-array (initial values from digits of pi)
// =============================================================================

/* prettier-ignore */
const BLOWFISH_S_ORIG: number[][] = [
  [
...
class Blowfish {
  ...
  encrypt(xl: number, xr: number): [number, number] {
    ...
    // If encipher modifies x in place.
...
// ... (sha512 unchanged)
// =============================================================================
// SHA-512 Implementation (for bcrypt_pbkdf)
// =============================================================================
...
function rotr64(x: bigint, n: number): bigint {
  return ((x >> BigInt(n)) | (x << BigInt(64 - n))) & SHA512_MASK;
}
...
function sha512(message: Uint8Array): Uint8Array {
  ...
  // Message needs to be padded to 896 bits mod 1024 (112 bytes mod 128)
  let padLen = 112 - ((msgLen + 1) % 128);
  if (padLen < 0) {
    padLen += 128;
  }
  ...
  for (let offset = 0; offset < padded.length; offset += 128) {
    ...
    for (let i = 16; i < 80; i++) {
      const s0 =
        rotr64(w[i - 15], 1) ^ rotr64(w[i - 15], 8) ^ (w[i - 15] >> 7n);
      const s1 = rotr64(w[i - 2], 19) ^ rotr64(w[i - 2], 61) ^ (w[i - 2] >> 6n);
      w[i] = (w[i - 16] + s0 + w[i - 7] + s1) & SHA512_MASK;
    }
    ...
    for (let i = 0; i < 80; i++) {
      ...
      const ch = (e & f) ^ (~e & SHA512_MASK & g);
      const temp1 = (h + S1 + ch + SHA512_K[i] + w[i]) & SHA512_MASK;
      const S0 = rotr64(a, 28) ^ rotr64(a, 34) ^ rotr64(a, 39);
      const maj = (a & b) ^ (a & c) ^ (b & c);
      const temp2 = (S0 + maj) & SHA512_MASK;

      h = g;
      g = f;
      f = e;
      e = (d + temp1) & SHA512_MASK;
      d = c;
      c = b;
      b = a;
      a = (temp1 + temp2) & SHA512_MASK;
    }
